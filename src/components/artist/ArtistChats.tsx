...
import { createMessagesForArtist } from "@/actions/createMessagesForArtist";
import { fetchAllArtistChats } from "@/actions/fetchAllArtistChats";

const ArtistChats = ({ session }: { session: Session | null | undefined }) => {
  ...
  const fetchArtistId = async () => {
    try {
      ...
      console.log("this is the artist data", artist);
      if (artist) {
        setArtistUserId(artist.artist?.userId ?? "");
        fetchChats(artist.artist?.userId ?? "");
      }
    } catch (error) {
  ...
  async function fetchChats(artistUserId: string) {
    try {
      const userChats = await fetchAllArtistChats(artistUserId);
  ...
  const handleSendMessage = async (
    ...
    try {
      const response = await createMessagesForArtist(
        messageContent,
        artistId,
        customerId
      );
                  ...
                  >
                    <Dialog>
                      <DialogTrigger asChild>
                        ...
                        >
                          Customer : {chat.customer.user.firstName} {""}
